...
if (!empty($data->created_by)) {
    ...
    $newtoken = $token->createToken2();


    if ($newtoken['outputStatus'] == 1000 && $newtoken['output'] == true) {

        http_response_code(201);
        echo json_encode(["message" => "token was created successfully", "status" => 1]);
        return;

    } elseif ($newtoken['outputStatus'] == 1100 && $newtoken['output'] == false) {

        http_response_code(203);
        echo json_encode(["message" => "token was NOT created. Please try again.", "status" => 0]);
        return;
